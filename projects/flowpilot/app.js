...
document.getElementById('test-flow').onclick=event=>{event.currentTarget.disabled=true;event.currentTarget.textContent='Running…';setTimeout(()=>{run({name:'New customer welcome flow',action:'Welcome email sent'});event.currentTarget.disabled=false;event.currentTarget.textContent='▶ Run test'},1200)};
render();
